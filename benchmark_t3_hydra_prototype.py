...
from chatterbox.models.t3.inference.hydra_decode import run_hydra_speculative_decode
...
from chatterbox.models.t3.train import load_hydra_heads_from_checkpoint
...
def run_timed_speculative(
    ...
    reset_cuda_peak_stats(device)
    started = time.perf_counter()
    result = run_hydra_speculative_decode(
        target_t3,
        hydra_model,
        request,
...
def main():
    parser = argparse.ArgumentParser(
        description="Prototype Hydra decoding on the current multilingual T3 using grounded future heads."
    )
    ...
    parser.add_argument("--speculate-k", type=int, default=3)
    parser.add_argument("--hydra-checkpoint-dir", required=True)
    ...
    hydra_model = load_hydra_heads_from_checkpoint(
        ...
        checkpoint_dir=args.hydra_checkpoint_dir,
        freeze_base=True,
    )

    proposal_cap = min(args.speculate_k, hydra_model.hydra_num_heads + 1)
    ...
    if args.output_dir and baseline_tokens is not None and speculative_tokens is not None:
        ...
        speculative_path = output_dir / "speculative_hydra.wav"
    ...
    print("benchmark=t3_hydra_prototype")
    print("mode=hydra_grounded_greedy_cfg_on_alignment_off")
    ...
    print(f"hydra_num_heads={hydra_model.hydra_num_heads}")
    print(f"hydra_num_layers={hydra_model.hydra_num_layers}")
